...
import { caseLawIndex } from '@/data/case-law.ts';
import { statuteIndex } from '@/data/statutes.ts';
import { scenarioChecklistsData } from '@/data/field-procedures/scenario-checklists.ts';
...
const statuteItems: SearchableItem[] = statuteIndex.map(s => ({
  ...
  keywords: [s.title, s.code, s.category, 'statute', 'law'],
}));
...
const scenarioItems: SearchableItem[] = Object.values(scenarioChecklistsData).map(s => ({
    ...
    keywords: [s.name, s.subtitle, s.category, ...s.keyStatutes, 'checklist', 'procedure'],
}));
