...
import type { BakerActGuide } from "@/data/emergency-response/baker-act-guide"
...
const CriteriaDetailView = ({ criteriaPoints }: { criteriaPoints: BakerActGuide['criteriaForInitiation']['english']['criteriaPoints'] }) => (
...
export const BakerActGuideClient = React.memo(
  function BakerActGuideClient({
    guideData,
  }: {
    guideData: BakerActGuide;
  }) {
    return (
      <Accordion type="single" collapsible className="w-full space-y-4" defaultValue="part2">
        <Section title={guideData.comparisonWithMarchmanAct.title} icon={Users} value="part1">
          <p className="text-muted-foreground">{guideData.comparisonWithMarchmanAct.explanation}</p>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Feature</TableHead>
                <TableHead className="text-accent">Baker Act</TableHead>
                <TableHead>Marchman Act</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {guideData.comparisonWithMarchmanAct.table.map((row) => (
                <TableRow key={row.feature}>
                  <TableCell className="font-medium">{row.feature}</TableCell>
                  <TableCell className="text-accent">{row.bakerAct}</TableCell>
                  <TableCell>{row.marchmanAct}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
