...
const DetailView = React.memo(({ detail }: { detail: FacDetail }) => (
  <div className="space-y-4">
    <div className="flex justify-between items-center">
        ...
        <Badge variant="secondary">{detail.governingAgency}</Badge>
    </div>
    ...
    <Alert className="bg-accent/20 border-accent/50">
      ...
      <AlertTitle>{detail.enforcementGuidance.title}</AlertTitle>
      <AlertDescription>
        <ul className="list-disc pl-5 space-y-1">
          {detail.enforcementGuidance.points.map((point, i) => <li key={i}>{point}</li>)}
        </ul>
      </AlertDescription>
    </Alert>
...
export const FloridaAdministrativeCodeClient = React.memo(function FloridaAdministrativeCodeClient({
  ...
  initialPlaceholders: FacPlaceholder[];
}) {
  const [activeItem, setActiveItem] = React.useState<string | undefined>();
  const [cachedDetails, setCachedDetails] = React.useState<Record<string, FacDetail>>({});
  ...
  const handleAccordionChange = async (value: string | undefined) => {
    ...
    try {
      const result = await analyzeFacRule({ ruleId: value });
      setCachedDetails(prev => ({ ...prev, [value]: result }));
    } catch (e) {
      console.error(e);
      setError(prev => ({ ...prev, [value]: "Failed to load F.A.C. rule analysis. Please try again." }));
    } finally {
      setLoadingId(null);
    }
  };
  ...
  const groupedPlaceholders = React.useMemo(() => {
    return initialPlaceholders.reduce((acc, placeholder) => {
      ...
      return acc;
    }, {} as Record<string, FacPlaceholder[]>);
  }, [initialPlaceholders]);
  
  const categoryOrder = [
    "Fish & Wildlife Conservation Commission (FWC)",
    "Dept. of Highway Safety & Motor Vehicles (DHSMV)",
    "Dept. of Business & Professional Regulation (DBPR - Alcoholic Beverages & Tobacco)"
  ];
