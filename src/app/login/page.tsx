...
export default function LoginPage({ isSignUp }: { isSignUp?: boolean }) {
    ...
    const router = useRouter()
    const { toast } = useToast()
    const { passkeysSupported, handlePasskeySignIn } = usePasskeys({ setIsLoading, toast, handleAuthError })
                ...
                <Card className="w-full max-w-md shadow-xl">
                    ...
                    <CardContent>
                        <Alert variant="destructive">
                            ...
                            <AlertTitle>System Configuration Issue</AlertTitle>
                            <AlertDescription>
                                The authentication system requiresconfiguration. Please contact your system administrator.
                            </AlertDescription>
                        </Alert>
                    </CardContent>
                </Card>
                            ...
                            <CardContent className="space-y-6">
                                ...
                                <Tabs defaultValue={isSignUp ? "sign-up" : "sign-in"} className="w-full">
                                    ...
                                    <TabsContent value="sign-in" className="mt-6 space-y-4">
                                        ...
                                        <Button onClick={handleGoogleSignIn} disabled={!!isLoading} variant="outline" className="w-full border-border hover:bg-muted/50">
                                            {isLoading === 'google' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <GoogleIcon />}
                                            Sign in with Google
                                        </Button>

                                        {passkeysSupported && (
                                            <Button onClick={handlePasskeySignIn} disabled={!!isLoading} variant="outline" className="w-full border-border hover:bg-muted/50">
