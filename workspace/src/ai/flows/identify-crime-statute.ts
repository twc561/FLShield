...
Follow these steps with a chain-of-thought process:
1.  **Analyze the user's crime description:** Break down the user's query into its core legal components (e.g., for "Man hit another man with a bat," the components are unwanted touching, use of a weapon, and causing injury).
...
3.  **Select the Statute:** Search your knowledge of Florida Statutes and select the single most specific statute that matches these concepts. For "Battery" + "Deadly Weapon," the correct statute is Aggravated Battery (F.S. 784.045).
